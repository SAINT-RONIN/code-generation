...
public class IbanGenerator {
    ...
    public static String generate() {
        String accountNumber = randomNineDigits();
        int checkDigits = calculateCheckDigits(BANK_CODE + accountNumber);
        return String.format("%s%02d%s%s", COUNTRY_CODE, checkDigits, BANK_CODE, accountNumber);
    }

    private static String randomNineDigits() {
        return String.format("%09d", ThreadLocalRandom.current().nextInt(1, 1_000_000_000));
    }

    private static int calculateCheckDigits(String bban) {
