...
async function doWithdraw(amount) {
  ...
  loading.value = true
  try {
    await atmWithdraw({ iban: selectedIban.value, amount })
  ...
  <AtmShell :showLogout="true" @logout="logout">
    <div class="flex flex-col items-center justify-center h-full p-10">
      <div v-if="done" class="text-center">
        ...
        <h1 class="font-display text-3xl mb-2" style="font-weight: 400;" :style="{ color: 'var(--ink)' }">Please take your cash.</h1>
        <p class="text-4xl font-display tabnum mb-2" style="font-weight: 300;" :style="{ color: 'var(--accent)' }">{{ eur(withdrawnAmount) }}</p>
      ...
      <div v-else class="w-full max-w-sm">
        ...
        <h1 class="font-display text-3xl mb-6" style="font-weight: 400;" :style="{ color: 'var(--ink)' }">Withdraw</h1>
        ...
        <div class="grid grid-cols-4 gap-2 mb-4">
          <button v-for="amt in PRESETS" :key="amt" class="h-14 rounded-xl text-sm font-medium lift border" :style="{ background: 'var(--surface)', borderColor: 'var(--line-2)', color: 'var(--ink)' }" @click="doWithdraw(amt)">€{{ amt }}</button>
        </div>
        <div class="flex gap-2">
          ...
          <button class="h-11 px-4 rounded-xl text-sm font-medium lift" :style="{ background: 'var(--accent)', color: 'var(--accent-ink)' }" :disabled="loading" @click="doWithdraw(parseFloat(customAmount))">Withdraw</button>
        </div>
      </div>
    </div>
  </AtmShell>
